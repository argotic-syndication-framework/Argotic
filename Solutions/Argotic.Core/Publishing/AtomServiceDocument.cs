namespace Argotic.Publishing
{
    ...
    [MimeMediaType(
        Name = "application",
        SubName = "atomsvc+xml",
        Documentation = "http://bitworking.org/projects/atom/rfc5023.html#iana-atomsvc")]
    public class AtomServiceDocument : ISyndicationResource, IExtensibleSyndicationObject, IAtomCommonObjectAttributes
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AtomServiceDocument"/> class using the supplied <see cref="Collection{T}"/> collection.
        /// </summary>
        ...
        public IEnumerable<AtomWorkspace> Workspaces
        {
            get
            {
                if (this.documentWorkspaces == null)
                {
                    this.documentWorkspaces = new Collection<AtomWorkspace>();
                }
        ...
        /// <exception cref="ArgumentNullException">The <paramref name="source"/> is a null reference (Nothing in Visual Basic).</exception>
        /// <exception cref="FormatException">The <paramref name="source"/> data does not conform to the expected syndication content format. In this case, the document remains empty.</exception>
        public static AtomServiceDocument Create(
            Uri source,
