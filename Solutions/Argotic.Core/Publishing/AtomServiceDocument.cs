...
public class AtomServiceDocument : ISyndicationResource, IExtensibleSyndicationObject, IAtomCommonObjectAttributes
{
    ...
    {
        return AtomServiceDocument.Create(source, new(), settings);
    }
    ...
    {
        return AtomServiceDocument.Create(source, new(credentials, proxy), settings);
    }
    ...
    private static void AsyncLoadCallback(IAsyncResult result)
    {
        ...
        if (result.IsCompleted)
        {
            ...
            if (parameters[1] is AtomServiceDocument document)
            {
