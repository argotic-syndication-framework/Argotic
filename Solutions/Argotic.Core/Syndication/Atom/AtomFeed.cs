...
public class AtomFeed : ISyndicationResource, IAtomCommonObjectAttributes, IExtensibleSyndicationObject
{
    ...
    /// <summary>
    /// Private member to hold references from the feed to one or more Web resources.
    /// </summary>
    ...
    public AtomEntry this[string id]
    {
        get
        {
            ...
            Uri uri = new(id, UriKind.RelativeOrAbsolute);
            ...
            foreach (AtomEntry entry in this.Entries)
            {
                if (entry.Id != null)
                {
                    ...
                    {
                        idUri = new(entry.Id.BaseUri, entry.Id.Uri);
                    }
                    else
                    {
                        idUri = entry.Id.Uri;
    ...
    {
        return AtomFeed.Create(source, new(), settings);
    }
    ...
    {
        return AtomFeed.Create(source, new(credentials, proxy), settings);
    }
    ...
    private static void AsyncLoadCallback(IAsyncResult result)
    {
        ...
        if (result.IsCompleted)
        {
            ...
            if (parameters[1] is AtomFeed feed)
            {
