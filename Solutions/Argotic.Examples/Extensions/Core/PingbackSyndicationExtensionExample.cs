...
/// <summary>
/// Reads a <see cref="PingbackSyndicationExtension"/> out of a feed the framework has already parsed, then writes the feed back out.
/// </summary>
/// <remarks>
...
/// </remarks>
internal static class PingbackSyndicationExtensionExample
{
    /// <summary>
    ...
    public static void ClassExample()
    {
        ...
        {
            if (feed.Channel.FindExtension(PingbackSyndicationExtension.MatchByType) is PingbackSyndicationExtension channelExtension)
            {
        ...
        foreach (RssItem item in feed.Channel.Items)
        {
            ...
            {
                if (item.FindExtension(PingbackSyndicationExtension.MatchByType) is PingbackSyndicationExtension itemExtension)
                {
    ...
    /// <summary>
    /// Builds a <see cref="PingbackSyndicationExtension"/> from scratch and reads it back.
    /// </summary>
    public static void AuthorExample()
    {
        ...
        PingbackSyndicationExtension pingback = new();
        pingback.Context.Server = new Uri("https://endjin.com/xmlrpc");
        pingback.Context.Target = new Uri("https://endjin.com/what-we-think/talks/rxdotnet-v7-0-released");
        item.Extensions.Add(pingback);
        ...
        if (readItem.FindExtension(PingbackSyndicationExtension.MatchByType) is PingbackSyndicationExtension readBack)
        {
            ExampleOutput.ShowPingbackExtension(readBack);
        }
    }
}