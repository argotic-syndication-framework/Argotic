...
public class FeedSynchronizationSharingInformation : IComparable<FeedSynchronizationSharingInformation>, IEquatable<FeedSynchronizationSharingInformation>, IComparisonOperators
{
    ...
    public int CompareTo(FeedSynchronizationSharingInformation? other)
    {
        ...
        if (result == 0) result = FeedSynchronizationSharingInformation.CompareSequence(this.Relations, other.Relations);
        if (result == 0) result = string.Compare(this.Since, other.Since, StringComparison.OrdinalIgnoreCase);
        if (result == 0) result = string.Compare(this.Until, other.Until, StringComparison.OrdinalIgnoreCase);
    ...
    public override int GetHashCode() => HashCode.Combine(HashCodeUtility.Component(this.ExpiresOn), HashCodeUtility.Component(this.Since), HashCodeUtility.Component(this.Until));
