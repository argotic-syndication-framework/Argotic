...
/// </summary>
/// <remarks>
///     A rating is a term plus the scheme it came from, and the term means nothing without the scheme. Absent a
///     <see cref="Scheme"/> the term is from <see cref="SimpleScheme"/>, whose whole vocabulary is
///     <see cref="SimpleAdultRating"/> and <see cref="SimpleNonAdultRating"/>; a media object with no rating at
///     all is unrestricted.
/// </remarks>
public class YahooMediaRating : IComparable<YahooMediaRating>, IEquatable<YahooMediaRating>, IComparisonOperators
{
    ...
    /// <exception cref="ArgumentNullException">The <paramref name="audience"/> is <see langword="null"/>.</exception>
    /// <exception cref="ArgumentException">The <paramref name="audience"/> is an empty string.</exception>
    ...
    /// </summary>
    /// <value><c>adult</c>, one of the two terms <see cref="SimpleScheme"/> defines.</value>
    public static string SimpleAdultRating => "adult";

    /// <summary>
    /// Gets the media simple rating scheme non-adult content rating.
    /// </summary>
    /// <value><c>nonadult</c>, one of the two terms <see cref="SimpleScheme"/> defines.</value>
    public static string SimpleNonAdultRating => "nonadult";

    /// <summary>
    /// Gets the media simple rating scheme.
    /// </summary>
    /// <value>The scheme a rating with no <see cref="Scheme"/> belongs to: <c>urn:simple</c>.</value>
    public static Uri SimpleScheme => new("urn:simple");

    /// <summary>
    /// Gets or sets the permissible audience for this media object.
    /// </summary>
    /// <value>The rating term, trimmed, interpreted against <see cref="Scheme"/>. The default value is an <i>empty</i> string, which is the one value a set operation cannot produce.</value>
    ...
    /// </summary>
    /// <value>The vocabulary <see cref="Content"/> is drawn from. The default value is <see langword="null"/>, which means <c>urn:simple</c>.</value>
    /// <remarks>
    ///     The inference is the caller's to make. <see langword="null"/> is kept distinct from
    ///     <see cref="SimpleScheme"/> so that saving does not write a <c>scheme</c> attribute the publisher
    ///     omitted.
    /// </remarks>
    ...
    /// <returns><see langword="true"/> if the <see cref="YahooMediaRating"/> was initialized using the supplied <paramref name="source"/>; otherwise, <see langword="false"/>.</returns>
    /// <remarks>
    ...
    /// <returns><see langword="true"/> if the specified <see cref="YahooMediaRating"/> is equal to the current instance; otherwise, <see langword="false"/>.</returns>
